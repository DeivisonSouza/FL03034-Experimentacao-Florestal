...
knitr::kable(
  tibble::tibble(
    ...
    filter(!stringr::str_detect(Slide,                                    "_files/|_cache/|assets")
                  ) %>%
    mutate(Slide = stringr::str_remove(Slide, "Slides/")
) %>%
  mutate(
    Temática = case_when(
      str_detect(Slide, "Progr") ~ "Programação da Disciplina",
      str_detect(Slide, "Introd") ~ "Introdução à Experimentação",
      str_detect(Slide, "AED") ~ "Análise Exploratória de Dados",
      str_detect(Slide, "DIC") ~ "Delineamento Inteiramente Casualizado",
      str_detect(Slide, "DBC") ~ "Delineamento em Blocos Casualizado",
      str_detect(Slide, "DQL") ~ "Delineamento em Quadrado Latino",
      .default = "A educação é um ato de amor, por isso, um ato de coragem. (Paulo Freire)"
    ), .after = Slide
  )
)
