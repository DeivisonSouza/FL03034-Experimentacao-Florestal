...
[1 - Competição de procedências florestais (Pimentel-Gomes; Garcia, 2002)](#proc)
...
- Existe alguma procedências que apresentou maior produção volumétrica?
]
]
...
<br>
.font90[
Do quadro de dados pode-se obter medidas como .blue[somatórios] e .blue[médias aritméticas] dos volumes de madeira para cada procedência e a soma para cada bloco.
]
...
data_pivot <- data %>%
  ...
  rename(Repeticao = `Bloco/Procedencia`) %>% 
  arrange(Procedencia, Repeticao)
```
...
res.aov <- aov(Valor ~ Procedencia, data = data_pivot)
```
