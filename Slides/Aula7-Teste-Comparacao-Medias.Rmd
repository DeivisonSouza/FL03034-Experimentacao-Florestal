...
.pull-left-4[
.font80[
**Parte 1 - Testes de Comparações de Médias**
...
[2 - Teste de comparações múltiplas de Tukey](#tt)

&nbsp;&nbsp;[2.1 - Diferença Mínima Significativa](#dms)

&nbsp;&nbsp;[2.2 - Hipóteses estatísticas do Teste de Tukey](#hett)

&nbsp;&nbsp;[2.3 - Etapas para realização do Teste de Tukey](#prtt)
...
.font200[**.yellow[Parte 1] <br> .white[Testes de Comparações <br> de Médias]**] 
...
.pull-left-4[
.font80[
**Antes de aprofundar sobre ".blue[Testes de Comparações de Médias]" vamos resgatar algumas ideias discutidas até aqui...**
]
<br>

.font70[
- Um .blue[experimento] é conduzido sob algum delineamento (DIC, DBC ou DQL) ou arranjo experimental para .blue[avaliar os efeitos de tratamentos] sobre uma variável resposta "y" de interesse.
- Para isso, .blue[dados] da variável resposta "y" .blue[são coletados], e estes são usados para avaliar as evidências e realizar inferências.
- Após uma boa análise exploratória, uma abordagem estatística comum é a realização de uma .blue[ANOVA], cujo modelo estatístico depende do delineamento experimental.
- Na ANOVA, o .blue[teste F] é usado para avaliar, sob a ótica estatística, a existência de evidências de .blue[igualdade entre as médias de tratamentos] (Hipótese H<sub>0</sub>).
]
]
...
- Se o valor da .blue[estatística F] (F<sub>Calculado</sub>) for **maior** do que o .blue[valor crítico na tabela da distribuição F-Snedecor], existem evidências para rejeitar H<sub>0</sub>.
...
- Se o valor da .blue[estatística F] (F<sub>Calculado</sub>) for **menor ou igual** do que o valor do (F<sub>Crítico</sub>), não existe evidências para rejeitar H<sub>0</sub>.
...
**Note**: .blue[O teste F não fornece evidências sobre qual é o melhor tratamento].
]
]
...
- Para responder essas perguntas foram idealizados os .blue[testes de comparações de médias].
- A ideia básica é realizar .blue[comparações par-a-par] entre as médias dos tratamentos, e inferir se estas são significativamente diferentes (ou não).
...
- **Ideia básica**: realizar .blue[comparações par-a-par] entre as médias dos tratamentos, baseando-se na .blue[Diferença Mínima Significativa] (DMS).
...
.pull-left-3[
.font80[
**Etapa 1**: Execute a ANOVA e avalie o teste F nos dados experimentais. .blue[Se a hipótese H<sub>0</sub> do teste F for rejeitada], proceda a realização de testes de comparações de médias.
<br>
...
.pull-right-3[
<br><br><br>
![STEPS](https://media4.giphy.com/media/Uu5z1AH8hEEG12YMGG/giphy.gif)  
