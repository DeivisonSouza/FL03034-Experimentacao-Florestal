...
```{r sumaov, echo=T, eval=T, collapse=FALSE}
# Resumo da análise de variância
summary(res.aov)
```
...
.font70[
- *P-valor < * $\alpha$ (2,46e-05 < 0,05): Rejeita-se a hipótese de nulidade (H<sub>0</sub>) do teste F.
- O *P-valor* foi menor do que o nível de significância $\alpha = 0,05$ (2,46e-05 < 0,05). Portanto, existem evidências de que as médias de pelo menos dois tratamentos (Substratos) diferem estatísticamente, ao nível de 5% de probabilidade.
]
]
