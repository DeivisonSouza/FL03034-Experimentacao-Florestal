...
(**Lembre-se**: A resolução manual da ANOVA DIC e do Teste de Tukey deste experimento foi apresentada nas aulas precedentes.)
...
<br>
.font90[
1) Carregue o conjunto de dados e análise da estrutura dos dados. (Obs.: Use a função `str()`. As variáveis categoricas são da classe *factor*?)

2) Realizar uma análise exploratória dos dados experimentais (quantitativa e visual).

3) Realizar a ANOVA do experimento em DIC e interpretar o Teste F-Snedecor (experimente $\alpha$ = 0,01 e 0,05).

4) Realizar a análise de pressupostos.

5) Realizar os testes de comparações de médias (Tukey e Duncan).
<br><br>
...
.font90[**Importante**: para aplicar as funções do R-base e outras funções é necessário converter os dados para o formato longo (empilhar os dados!)]
...
library(readr)
(data <- 
    read_csv("data/DIC-Pimentel-Gomes-2002-Org.csv", 
             col_types = "ffd"))
```
...
.pull-top[
.font90[
- **Passo 2** - Obter um resumo estatístico por tratamento (Progênie)...
...
stby(data = data, 
     INDICES = data$Progenie, 
     FUN = descr, 
     stats = "common", # c("mean", "sd")
     transpose = TRUE)
```
...
.pull-left-4[
.font80[
- **Passo 2** - Obter um resumo estatístico por tratamento (Progênie)...(BoxPlot por Progênie)

```{r box, echo=TRUE, eval=FALSE}
# Criando um BoxPlot Por Progênie
g1 <- data %>% 
  ggplot(aes(x = reorder(Progenie, -d), y = d)) + 
  geom_boxplot(mapping = aes(fill = Progenie)) + 
  ...
  scale_y_continuous(name = "Diâmetro (cm)", 
                     breaks = seq(5, 18, 2),
                     limits = c(5, 18)) +
  labs(x = "Progênie")
...
.pull-left-4[
.font80[
- **Passo 2** - Obter um resumo estatístico por tratamento (Progênie)...(Gráfico de barra por Progênie)

```{r bar, echo=T, eval=F}
data |>
  ggplot(
    mapping = aes(x = Progenie, 
                  y = d)) +
  geom_bar(
    mapping = aes(fill = Progenie),
    stat = "summary",
    fun = mean
    ) +
...
xyplot(d ~ reorder(Progenie, -d),
       data = data,
       type = c("p", "a"),
       ylab = expression("Diâmetro (cm)"),
       xlab = expression("Progênie"))
```
]
]
