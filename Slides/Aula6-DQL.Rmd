...
df2 %>%
  mutate(
    C1 = cell_spec(C1, "html", color = ifelse(C1 == 108.44, "red", "black")),
    C2 = cell_spec(C2, "html", color = ifelse(C2 == 51.69, "red", "black")),
    C3 = cell_spec(C3, "html", color = ifelse(C3 == 59.36, "red", "black")),
    C4 = cell_spec(C4, "html", color = ifelse(C4 == 88.78, "red", "black")),
    C5 = cell_spec(C5, "html", color = ifelse(C5 == 29.29, "red", "black"))
  ) %>%
  kable("html", escape = F) %>%
  kable_styling("striped", full_width = F)

#reshape2::dcast(df, linha ~ colun, value.var = "prod")
```
...
.font80[
- **Gráfico Dot Plot**: Ajuda a visualizar como a produção variou nas parcelas experimentais para as diferentes cultivares.
]

.pull-left-1[
.font80[
```{r dot, echo=T, eval=F, collapse=T}
df <- labestData::DiasEg9.4

df %>%
  ggplot(aes(x=forcats::fct_reorder(cult, desc(prod), .fun='mean'),
             y=prod, fill=cult)) +
...
.pull-right-2[
```{r ref.label="dot", echo=FALSE, eval=TRUE, collapse=T, fig.width=5, fig.height=2.7}
```
]
...
df2 %>%
  mutate(
    ...
    C3 = cell_spec(C3, "html", color = if_else(C3 == 59.36, "red", "black")),
    C4 = cell_spec(C4, "html", color = if_else(C4 == 88.78, "red", "black")),
    C5 = cell_spec(C5, "html", color = if_else(C5 == 29.29, "red", "black"))
  ) %>%
  kable("html", escape = F) %>%
  kable_styling("striped", full_width = F) %>% 
  row_spec(1:4, color = 'black', background = 'white') %>% 
