...
\begin{equation}
\Large
SQ_{Tot.} = \sum_{i=1}^{p}\sum_{j=1}^{p}\sum_{k=1}^{p}y_{ijk}^2 - \frac{1}{p^2}\left ( \sum_{i=1}^{p}\sum_{j=1}^{p}\sum_{k=1}^{p}y_{ijk} \right )^2
\end{equation}
$$
...
- Em outras palavras, mede o desvio de uma observação y<sub>ijk</sub> em relação à média amostral geral, sem se importar com qual tratamento ou bloco (linha ou coluna) essa observação pertence.
]
]
